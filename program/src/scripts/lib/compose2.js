...
;(function(){
	...
	var compose = {
		...
		initLoad: function(){
			...
			if(main){
				...
				var head = document.head || document.getElementsByTagName('head')[0],
					element = this.createTag('script', path);
		 ...
		 */
		require: function(pathId, requires, callback, fail, existObjectNames){
			this.req(pathId, requires, callback, fail, existObjectNames);
		},
		/**
		 * 异步请求资源
		 */
		async: function(pathId, requires, callback, fail, existObjectNames){
			this.req(pathId, requires, callback, fail, existObjectNames, {type:'async'});
		},
		/**
		 * 并发请求资源,建议加载css资源等
		 */
		paral: function(pathId, requires, callback, fail){
			this.req(pathId, requires, callback, fail, null, {type:'parall'});
		},
		...
		req: function(pathId, requires, callback, fail, existObjectNames, opts){
			...
			if(opts.type === 'parall'){
				...
				this.handlingParamRequires(requires);
				return;
			}
			else if(this.isWaitingForChild() == true && opts.type !== 'async'){
			...
			else if(this._handlingRequire && opts.type === 'async'){
				requires = this.preHandleRequires(requires);
				return;
			}
			requires = this.preHandleRequires(requires);
		 ...
		 * 满足
		 */
		satisfy: function(resourceObject, fail){
		 ...
		 * 预处理
		 */
		preHandleRequires: function(requires, parall){
			...
			for(var j=0; j<=requires.length; j++){
				...
				if(!parall)_requires.push(require);
			}
			if(pathId){
		...
		handlePath: function(pathId, contextPath, basePath, param){
			...
			if(param){
				path += require.path.match(/\.js\?/) ? require.path.match(/\?$/) ? param : '&' + param : '?'+ param;
			}
			return path;
		},
		/**
		 ...
		 * 处理单个require
		 */
		handleRequire: function(require, parall){
			//对处理过的需求直接返回
			...
			if(!require.handled && require.callback && !parall){
				this.checkComplete(require);
			}
			else if( !parall && (!require.path || require.resourceObject || (require.handled == true && require.type === 'css'))){
				this.excCompleteAndExcNext(require);
			}
			else{
				var element, self = this, config = this._config;
				element = this.createTag(require.type, require.path);

				if(this._config.dependOnload && !parall){
					addOnloadEvent(element, function(status){
						self.satisfy(null, status);
					}, function(status){
						self.satisfy(null, status);
					})
				}
		 ...
		 * 设置当前处理对象
		 */
		handlingParamRequires: function(requires){
			if(requires){
				var l = requires.length;
				for(var i=0; i<l; i++){
					this.handleRequire(requires[i]);
				}
				if(requires.callback)requires.callback();
			}
		}
	}
	compose.initConfig();
