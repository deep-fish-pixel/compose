...
;(function(){
	...
	var compose = {
		...
		req: function(pathId, requires, callback, fail, existObjectNames, opts){
			...
			var handling = this._handlingRequire;
			if(opts.method === 'paral'){
				this.preHandleRequires(requires);
				//this.handlingParamRequires(requires);
				this.handleRequires();
				return;
			}
			else if(this.isWaitingForChild() == true && opts.method !== 'async'){
				requires.parent = handling;
				this.preHandleRequires(requires);
				this.handleRequires();
				return;

			}
			else if(handling && handling.parent){
				requires.parent = handling.parent;
				this.preHandleRequires(requires);
				return;
			}
			else if(handling && !handling.handled && opts.method === 'async'){
				this.preHandleRequires(requires);
				return;
			}
			this.preHandleRequires(requires);
		...
		handleRequire: function(require){
			...
			if(!require.handled && require.callback && !paral){
				this.checkComplete(require);
			}
			else if( !paral && (!require.path || require.resourceObject || (require.handled == true && require.type === 'css'))){
				this.excCompleteAndExcNext(require);
			}
			else{
		...
		triggerLoad: windowLoader.triggerLoad,
		/**
